...
ECG from the custom hardware is noisier than Polar H10, so we apply a tighter
bandpass filter (0.5-25 Hz, 4th-order Butterworth) before analysis.
...
from serial_ecg.utils.har_inference import HARInferenceEngine
...
_5S_ECG_SAMPLES  = ECG_NATIVE_HZ * 5    # 500
_5S_ACC_SAMPLES  = ACC_HZ * 5           # 500
_10S_ACC_SAMPLES = ACC_HZ * 10          # 1000
_30S_ECG_SAMPLES = ECG_NATIVE_HZ * 30   # 3 000
...
class ProcessingWorker(QObject):
    """
    Consumes raw 100 Hz ECG + 100 Hz ACC data and periodically runs analysis.

    Key difference from the Polar version: the ECG bandpass is 0.5-25 Hz
    (instead of 0.5-40 Hz) because the custom hardware signal is noisier.
    ...
    def run(self):
        self._running = True
        self.status.emit("Processing worker started (100 Hz ECG, 0.5-25 Hz bandpass)")
    ...
    def _compute_5s_window(self, ecg_5s: np.ndarray, acc_xyz, acc_10s=None) -> dict:
        ...
        # 0.5-25 Hz bandpass (tighter than Polar's 0.5-40 Hz for noisier ECG)
        b, a = scipy.signal.butter(4, [0.5, 25], btype='bandpass', fs=ECG_NATIVE_HZ)
    ...
    def _compute_hrv(self, signal: np.ndarray) -> dict:
        ...
        # Apply the same 0.5-25 Hz bandpass before HRV analysis
        b, a = sig.butter(4, [0.5, 25], btype='bandpass', fs=ECG_NATIVE_HZ)
